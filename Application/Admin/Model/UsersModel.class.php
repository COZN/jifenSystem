...
class UsersModel extends BaseModel {
    /**
	 ...
	 public function editAccount(){
	 	...
	 	if(I('payPwd')!='')$m->payPwd = md5(I('payPwd').$loginSecret);
    ...
    public function findByUserId()
    {
        ...
        return $this->field("userMoney,userId,userType")->where("userId = {$userid}")->find();
    }
    /**
     * 更新余额且生成消费记录
     * @return bool|mixed
     */
    public function updateMoneyByUserId()
    {
        $money = I('post.userMoney','','float');
        $userType = I('post.userType','','intval');
        $userId = I('post.userId','','intval');
        $addMoney = I('post.addMoney','','float');
        $ReduceMoney = I('post.ReduceMoney','','float');
        ...
        }else if($ReduceMoney !== '')
        {
            ...
            if($money <= 0 ||($data['userMoney']) < 0)
            {
                return false;
            }
        }

        if( $this->where(" userType = {$userType} and userId = {$userId}")->save($data))
        {
            /*如果成功增加余额变动记录
            $m = M('MoneyRecord');
            $info['actionType'] = 9;
