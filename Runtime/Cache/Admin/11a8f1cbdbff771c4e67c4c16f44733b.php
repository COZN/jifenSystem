        ...
        <table class="table table-hover table-striped table-bordered wst-list">
           ...
           <tbody>
            <?php if(is_array($Page['root'])): $i = 0; $__LIST__ = $Page['root'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
               ...
               <?php if($object['workStatus'] ==0 ): ?>离职<?php endif; ?>&nbsp;-->
                   <?php if($vo['workStatus'] ==1){echo '在职';}else{echo '离职';} ?>
               </td>
               ...
               <?php if($vo['staffId'] != $Think.session.STAFF.staffId): if($vo['loginName'] != 'admin' ): ?><button type="button" class="btn btn-default glyphicon glyphicon-trash" onclick="javascript:del(<?php echo ($vo['staffId']); ?>)"></button><?php endif; endif; ?>
               <?php } ?>
               </td>
