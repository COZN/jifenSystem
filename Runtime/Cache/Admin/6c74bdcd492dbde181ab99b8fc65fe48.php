...
<html lang="zh-cn">
   ...
   <script>
     ...
     function BatchDelete(t){
        ...
        Plugins.confirm({title:'信息提示',content:'您确定要删除这些申请记录吗?',okText:'确定',cancelText:'取消',okFun:function(){
            ...
            $.post("<?php echo U('Admin/Apply/BatchDelete');?>",{id:ids.join(','),page:t,activeId:activeId},function(data,textStatus){
   ...
   function toggleBackStatus(t,v,activePhase){
    ...
    function ExportSelected(exportStatus)
    {
      ...
      //location.href="/index.php/Admin/Apply/outExcel/applyId/"+jsonText;
   ...
   <body class='wst-page'>
     <form method='post' action="<?php echo U('Admin/Apply/record');?>">
       <div class='wst-tbar'> 
       <input type="hidden" id="activeId" value="<?php echo ($activeId); ?>">
       第
        <select name="activePhase" style="width:60px;">
          <?php if(is_array($allActive)): $i = 0; $__LIST__ = $allActive;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option <?php if($activePhase == $vo['activePhase']): ?>selected="selected"<?php endif; ?>><?php echo ($vo['activePhase']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
        </select>
       期  &nbsp;&nbsp;&nbsp;&nbsp;
       兑现状态：
