...
<html lang="zh-cn">
   ...
   function del(id){
	   Plugins.confirm({title:'信息提示',content:'您确定要删除该分类吗?',okText:'确定',cancelText:'取消',okFun:function(){
		   Plugins.closeWindow();
		   Plugins.waitTips({title:'信息提示',content:'正在操作，请稍后...'});
		   $.post("<?php echo U('Admin/LogIp/del');?>",{id:id},function(data,textStatus){
				var json = WST.toJson(data);
				if(json.status=='1'){
					Plugins.setWaitTipsMsg({content:'操作成功',timeout:1000,callback:function(){
					    location.reload();
   ...
   <body class='wst-page'>
       ...
       <a class="btn btn-success glyphicon glyphicon-plus" href="<?php echo U('Admin/LogIp/toEdit');?>" style='float:right'>新增</a>
        ...
        <table class="table table-hover table-striped table-bordered wst-list">
           ...
           <tbody>
            <?php if(is_array($List['root'])): $i = 0; $__LIST__ = $List['root'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id='tr_0_<?php echo ($i); ?>' class="tr_0" isLoad='1'>
             <td><?php echo ($i); ?></td>
			 <td>
				<input type='text' value='<?php echo ($vo['ip']); ?>' onchange='javascript:editName(this)' dataId="<?php echo ($vo["id"]); ?>" name="ip" class='form-control wst-ipt'/>
			 </td>
               ...
               <a class="btn btn-default glyphicon glyphicon-pencil" href="<?php echo U('Admin/LogIp/toEdit',array('id'=>$vo['id']));?>">修改</a>&nbsp;
