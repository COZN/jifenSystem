...
<html lang="zh-cn">
   ...
   /*function editName(obj){
	  ...
	  /!* var params = {};
	   ...
	   $.post("<?php echo U('Admin/LogIp/editName');?>",{id:$(obj).attr('dataId'),ip:obj.value,},function(data,textStatus){	//{id:$(obj).attr('dataId'),points_value:obj.value}
   ...
   function del(id){
	   Plugins.confirm({title:'信息提示',content:'您确定要删除该用户吗?',okText:'确定',cancelText:'取消',okFun:function(){
		   Plugins.closeWindow();
		   Plugins.waitTips({title:'信息提示',content:'正在操作，请稍后...'});
		   $.post("<?php echo U('Admin/Member/del');?>",{id:id},function(data,textStatus){
				var json = WST.toJson(data);
				if(json.status=='1'){
					Plugins.setWaitTipsMsg({content:'操作成功',timeout:1000,callback:function(){
					    location.reload();
   ...
   <body class='wst-page'>
       <div class='wst-tbar' style='text-align:right;height:25px;'>
       			<a class="btn btn-success glyphicon glyphicon-plus" href="<?php echo U('Admin/Member/toEdit');?>" style='float:right'>新增</a>
       </div>
       <div class='wst-body'> 
        <table class="table table-hover table-striped table-bordered wst-list">
           ...
           <tbody>
            <?php if(is_array($List['root'])): $i = 0; $__LIST__ = $List['root'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id='tr_0_<?php echo ($i); ?>' class="tr_0" isLoad='1'>
             <td width="50"><?php echo ($i); ?></td>
			 <td>
				<!--<input type='text' value='<?php echo ($vo['loginName']); ?>' onchange='javascript:editName(this)' dataId="<?php echo ($vo["id"]); ?>" name="ip" class='form-control wst-ipt'/>-->
				 <?php echo ($vo['loginName']); ?>
			 </td>
               ...
               <td>
				   <a class="btn btn-default glyphicon glyphicon-pencil" href="<?php echo U('Admin/Member/toEdit',array('id'=>$vo['userId']));?>">修改</a>&nbsp;
             ...
             <?php if($vo['childNum'] > 0): if(is_array($vo['child'])): $i2 = 0; $__LIST__ = $vo['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo2): $mod = ($i2 % 2 );++$i2;?><tr id='tr_0_<?php echo ($i); ?>_<?php echo ($i2); ?>' class="tr_0_<?php echo ($i); ?>" isLoad='1'>
	               ...
	               <a class="btn btn-default glyphicon glyphicon-trash" onclick="javascript:del(<?php echo ($vo2['catId']); ?>)">刪除</a>
