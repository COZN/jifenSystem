...
function editGoods(menuId){
	   ...
	   params.isGroup =$('input[name="isGroup"]:checked').val();;
	   params.groupPrice = $('#groupPrice').val();
	   params.groupMaxCount = $('#groupMaxCount').val();
	   params.groupMinCount = $('#groupMinCount').val();
	   params.startTime = $('#startTime').val()+$('#startTimeH').val()+':'+$('#startTimei').val()+':'+$('#startTimes').val();
	   params.endTime = $('#endTime').val()+$('#endTimeH').val()+':'+$('#endTimei').val()+':'+$('#endTimes').val();
	   params.groupDesc = $('#groupDesc').val();
...
function addPriceAttr(attrId,attrName,flag='6'){
	var goodsPriceNo = $('.hiddenPriceAttr').attr('dataNo');goodsPriceNo++;
	...
	html.push('<td><input type="radio" onclick="checkAttPrice('+obj.attrId+','+goodsPriceNo+');" id="price_isRecomm_'+obj.attrId+'_'+goodsPriceNo+'" name="price_isRecomm_'+obj.attrId+'"/></td>');
	html.push('<td><input type="text" id="price_stock_'+obj.attrId+'_'+goodsPriceNo+'" onblur="getTstock('+goodsAll+');" value="100" onkeypress="return WST.isNumberKey(event)" onblur="javascript:statGoodsStaock()" onkeyup="javascript:WST.isChinese(this,1)" maxLength="25"/></td>');
...
function toAddAttr(){
	...
	html.push("<td><input type='checkbox' name='isPriceAttr' id='isPriceAttr_"+attrNoForAttr+"' onclick='javascript:chkPriceAttrForAttr()' id='isPriceAttr_"+attrNoForAttr+"'></td>");
